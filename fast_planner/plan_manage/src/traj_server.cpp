...
vector<double> failPlan_traj(4, 0.0);  //规划失败的悬停信息，x,y,z,yaw
...
void bsplineCallback(plan_manage::BsplineConstPtr msg) {
  if(msg->order==-1) {  //no path，规划失败，悬停，废弃
    failPlan_traj[0] = msg->pos_pts[0].x;
    failPlan_traj[1] = msg->pos_pts[1].y;
    failPlan_traj[2] = msg->pos_pts[2].z;
    failPlan_traj[2] = msg->yaw_pts[0];
    traj_duration_ = -1;  //标志位，时间为-1，即规划失败
    return;
  }
  // parse pos traj
...
void replanCallback(std_msgs::Empty msg) {
  ...
  traj_duration_ = min(t_stop, max(traj_duration_, 0.0));
}
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  } else if(traj_duration_==-1) { //废弃
    ROS_INFO("position=(%0.2f,%0.2f,,%0.2f) direction=%0.2f", 
    failPlan_traj[0], failPlan_traj[1],failPlan_traj[2], failPlan_traj[3]);
    /* planning fail, hover and waiting for human command */
    for(int i=0; i<3; i++) {
      pos(i) = failPlan_traj[i];
    }
    vel.setZero();
    acc.setZero();
    yaw = failPlan_traj[3];
