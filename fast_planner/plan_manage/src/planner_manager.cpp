...
void FastPlannerManager::planYaw(const Eigen::Vector3d& start_yaw) {
  ...
  int half_pi_num = 30; // 旋转90度使用的点数
  auto&  pos      = local_data_.position_traj_;
  double duration = pos.getTimeSum();

  double dt_yaw  = 0.3;
  int    seg_num = half_pi_num+ceil(duration / dt_yaw);
  ...
  //指定偏航角，0：x轴方向，pi/2，逆时针90度(y轴方向)
  //前n个点用来调整航向
  //由于多次规划可能导致yaw跳动

  // 匀速旋转，每一个点转pi/(half_pi_num*2)
  double w_yaw = (M_PI/(2*half_pi_num));

  static double last_target = -1;
  int offset = 0;
  if(targetYaw==last_target) {  //避免第一次旋转角度过大
    offset = 1;
  }
  else {
    last_target = targetYaw;
  }
  yaw.setConstant(targetYaw);
  // cout<<"cur_yaw: "<<cur_yaw<<" targetYaw "<<targetYaw<<endl;
  //角度差与最短要旋转的角度差
  double diff_yaw = targetYaw-cur_yaw, hover_angle = 0;
  bool c_plus = true;
  //选择最短路旋转
  if(abs(diff_yaw)<2*M_PI-abs(diff_yaw)) {
    c_plus = true;
    hover_angle = diff_yaw;
  }
  else {
    c_plus = false;
    hover_angle = diff_yaw>0?2*M_PI-abs(diff_yaw):-2*M_PI+abs(diff_yaw);
  }
  double hover_num = min((double)(seg_num+3), abs(hover_angle)/w_yaw);  //旋转使用的点数，避免超出总数
  //修改负责旋转部分导航点的Yaw，其余不变为Targetaw
  for(int i=0; i<(int)hover_num-offset; i++) {  //可以不用保证yaw处于[-pi, pi]
    if(c_plus) yaw(i, 0) = hover_angle/(int)hover_num*(i+offset)+cur_yaw; 
    else yaw(i, 0) = -hover_angle/(int)hover_num*(i+offset)+cur_yaw; 
  }
