...
void TopoReplanFSM::init(ros::NodeHandle& nh) {
  ...
  replan_pub_  = nh.advertise<std_msgs::Empty>("/planning/replan", 20);// just a trigger signal, nothing to transform
  new_pub_     = nh.advertise<std_msgs::Empty>("/planning/new", 20);
  bspline_pub_ = nh.advertise<plan_manage::Bspline>("/planning/bspline", 20);
  requestTarget_pub_ = nh.advertise<std_msgs::Empty>("/planning/request_traget", 20);
}
...
void TopoReplanFSM::waypointCallback(const nav_msgs::PathConstPtr& msg) {
  ...
  if (target_type_ == TARGET_TYPE::REFENCE_PATH) {
    ...
    if (target_type_ == TARGET_TYPE::MANUAL_TARGET) {
      ...
      geometry_msgs::Quaternion orientation = msg->poses[0].pose.orientation;
      tf::Matrix3x3 mat(tf::Quaternion(orientation.x, orientation.y, orientation.z, orientation.w));
      double yaw, pitch, roll;
      mat.getEulerYPR(yaw, pitch, roll);
      planner_manager_->setTargetYaw(yaw);
...
void TopoReplanFSM::odometryCallback(const nav_msgs::OdometryConstPtr& msg) {
  ...
  //记录最新的Yaw
  tf::Matrix3x3 mat(tf::Quaternion(odom_orient_.x(), odom_orient_.y(), odom_orient_.z(), odom_orient_.w()));
  double yaw, pitch, roll;
  mat.getEulerYPR(yaw, pitch, roll);
  planner_manager_->setCurYaw(yaw);
...
void TopoReplanFSM::printFSMExecState() {
  ...
  if(exec_state_==WAIT_TARGET) {
    requestTarget_pub_.publish(std_msgs::Empty());
  }
}
...
bool TopoReplanFSM::callTopologicalTraj(int step) {
  bool plan_success;
  static Eigen::Vector3d fail_location;  //失败时的位置，在此悬停
  static bool first_fail = true;  //判断是否成功过，若是则更新最后的安全位置，进行悬停
  if (step == 1) {
  ...
  if (plan_success) {
    ...
    return true;
  } else {  //no path，发布位置，当飞机悬停，废弃，没用，相当于直接返回false
    if(first_fail) {
    ROS_INFO("set location=(%0.2f,%0.2f,,%0.2f", 
    odom_pos_(0), odom_pos_(1),odom_pos_(2));
      fail_location = odom_pos_;
      first_fail = false;
    }
    plan_manage::Bspline bspline;
    bspline.order  = -1;  //借用order做标志为，正常不可能为负数
    geometry_msgs::Point pt;
    pt.x = fail_location(0);
    pt.y = fail_location(1);
    pt.z = fail_location(2);
    bspline.pos_pts.push_back(pt);  
    bspline.yaw_pts.push_back(planner_manager_->getCurYaw());  //指定一个航向即可
    // bspline_pub_.publish(bspline);
    return false;
  }
}
